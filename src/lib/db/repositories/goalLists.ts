...
/**
 * Computes the next prepend-order value for a new goal list.
 *
 * Returns `min(existing orders) - 1` so new lists appear first when sorted
 * ascending.  Returns `0` if the user has no lists yet.
 *
 * @param userId - The owning user's identifier
 * @returns The order value to assign to the new list
 */
