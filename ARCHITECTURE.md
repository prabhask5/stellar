...
| Sync Engine | `@stellar/sync-engine` | Manages Dexie, Supabase, sync — see [engine docs](https://github.com/prabhask5/stellar-drive/blob/main/ARCHITECTURE.md) |
...
Both functions are imported from `stellar-drive/auth`.
...
**File**: `static/sw.js` *(auto-generated at build time by the `stellarPWA` Vite plugin from `stellar-drive/vite` — do not edit manually)*
...
> **Note:** Most of the sync, auth, conflict resolution, realtime, and network complexity listed above is now encapsulated in the `@stellar/sync-engine` package. See [stellar-drive/ARCHITECTURE.md](https://github.com/prabhask5/stellar-drive/blob/main/ARCHITECTURE.md) for detailed documentation of these subsystems.
